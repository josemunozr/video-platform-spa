const reducer = (state, action) => {
  const getResultSearch = (state, payload) => {
    let result = [];
    if (action.payload) {
      result = state.trends.filter((item) => item.title.toLowerCase().includes(action.payload.toLowerCase()));
      result = result.concat(state.originals.filter((item) => item.title.toLowerCase().includes(action.payload.toLowerCase())));
    }
    return result;
  };
  switch (action.type) {
